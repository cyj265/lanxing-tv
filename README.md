# 揽星 TV（LanXing TV · Android TV IPTV 播放器）

一个专为 **Android TV / 电视盒子** 设计的开源 IPTV 播放器，支持 **Android 6.0+（minSdk 23）**。

播放内核使用 **Media3 ExoPlayer 1.11.0**（Google 官方播放器内核），对 HLS 直播流兼容性最好，H.265/HEVC 高清源硬解。

> 以下界面为**示意预览**（非真机截图）。

| 全屏播放 | 频道列表（OK 键唤出） | 设置面板（菜单键唤出） |
| --- | --- | --- |
| ![全屏播放](docs/screenshot-play.svg) | ![频道列表](docs/screenshot-channels.svg) | ![设置面板](docs/screenshot-settings.svg) |

## 为什么选揽星 TV

| 功能 | 揽星 TV | 同类商业播放器 | 同类聚合播放器 | 同类轻量播放器 |
| --- | :---: | :---: | :---: | :---: |
| 开源免费 · 无广告 | ✅ | ❌（付费解锁） | ✅ | ✅ |
| 单频道多线路自动切换 | ✅ | ✅ | ❌ | ❌ |
| 超时自动换源 | ✅ | ✅ | ❌ | ❌ |
| 直播源健康度检测+自动优选 | ✅ | ❌ | ❌ | ❌ |
| 扫码局域网管理（手机加源） | ✅ | ❌ | ❌ | ❌ |
| EPG 节目单（正在播放/稍后） | ✅ | ✅ | ✅ | ❌ |
| 二级分组频道列表 | ✅ | ✅ | ✅ | ❌ |
| 画面比例（原始/16:9/4:3/裁剪/拉伸） | ✅ | ✅ | ✅ | 部分 |
| 解码模式（自动/硬解/软解） | ✅ | ✅ | ✅ | 部分 |
| 自动更新（直连+加速双线） | ✅ | ✅ | ❌ | ❌ |
| 崩溃日志一键导出 | ✅ | ❌ | ❌ | ❌ |
| 双击返回退出（结束进程） | ✅ | ❌ | ❌ | ❌ |
| Android 6.0+ 老盒子支持 | ✅ | 部分 | 部分 | 部分 |
| 安装包体积 | **~2.8MB** | ~30MB | ~50MB | ~10MB |

> 对比基于同类产品公开功能整理，仅供参考。

## 真机截图

> 以下为真机截图占位，后续替换为实际截图。

| 全屏播放 | 频道列表（二级分组） | 设置面板 |
| --- | --- | --- |
| ![全屏播放](docs/screenshot-real-play.svg) | ![频道列表](docs/screenshot-real-channels.svg) | ![设置面板](docs/screenshot-real-settings.svg) |

| 直播源管理 | 扫码局域网管理 | 节目单详情 |
| --- | --- | --- |
| ![直播源管理](docs/screenshot-real-source.svg) | ![扫码管理](docs/screenshot-real-qrcode.svg) | ![节目单](docs/screenshot-real-epg.svg) |

## 功能

- 📺 **M3U / M3U8 / TXT 播放列表**：支持 URL 加载，也支持从本地文件导入
- 📡 **多直播源管理**：可同时配置多个播放列表源，在设置面板中切换；每个源独立缓存，加载失败自动用上次缓存
- 🩺 **直播源健康度检测**：一键测速所有源的延迟，显示状态（正常/较慢/不可用）和延迟（ms）；支持"自动优选最快源"开关，测速后自动切换到延迟最低的可用源
- 📱 **扫码局域网管理**：机顶盒显示二维码，手机连同一 Wi-Fi 扫码即可在手机上添加/切换直播源、设置节目指南、直接播放
- 📅 **EPG 节目单**（XMLTV）：顶部显示"正在播放/稍后播放"，频道列表中直接显示每个台当前节目
- 🗂️ **二级分组频道列表**：左栏为央视/卫视等分组，右栏显示对应分组的频道；支持搜索、收藏
- 📶 **自动识别流格式**：HLS (m3u8) / MPEG-TS / MP4 等
- 🖥️ **始终全屏播放**：播放画面不被频道栏遮挡，悬浮层按需唤出
- 🔄 **上下键换台 / 左右键换线路**：全屏播放态上下键切换频道，左右键切换当前频道的多条播放线路
- ⏱️ **超时自动换源**：可设置超时时间（5~60秒），当前线路加载超时自动切换下一条线路
- 🔄 **检查更新自动下载安装**：读取 GitHub 最新 Release，高于当前版本自动下载并拉起系统安装器；**双线下载**：先直连，失败自动切换 gh-proxy 加速中转；配置与收藏保留
- 🛠️ **崩溃日志导出**：真机问题一键导出 txt 发给开发者分析；启动时提示一次（不重复弹窗），日志保留可在调试设置中查看/导出/清理
- 🎨 **画面比例**：原始 / 16:9 / 4:3 / 缩放裁剪 / 拉伸全屏
- 🎬 **解码模式**：自动 / 仅硬件 / 仅软件，自动模式下硬解失败自动回退软解
- 💾 **自动恢复**：打开应用自动续播上次频道
- 🚪 **双击返回退出**：连续按两次返回键弹出退出确认，退出后结束所有进程（后台无声音）
- 📱 **Android 6.0+ 全支持**（minSdk 23），老盒子也能装

## 遥控器操作

| 按键 | 功能 |
| --- | --- |
| 上 / 下 方向键 | 切换上 / 下一个频道（全屏播放态） |
| 左 / 右 方向键 | 切换当前频道的上 / 下一条播放线路 |
| OK / 确认键 | 唤出 / 收起频道列表（全屏播放态） |
| 菜单键 | 唤出 / 收起右侧设置面板 |
| CH+ / CH- | 切换上 / 下一个频道 |
| 播放 / 暂停键 | 播放 / 暂停 |
| 返回键（双击） | 弹出退出确认，退出后结束所有进程 |
| 返回键（单击） | 关闭已打开的悬浮面板 |

### 频道列表内操作

| 按键 | 功能 |
| --- | --- |
| 上 / 下 | 左栏分组中移动焦点，右栏频道随分组自动切换 |
| 右 | 焦点从左栏分组移到右栏频道列表 |
| 左 | 焦点从右栏频道移回左栏分组 |
| OK（右栏频道） | 播放选中的频道 |
| OK（左栏分组） | 展开/收起分组，焦点自动移到右栏第一个频道 |

### 设置面板内操作

| 按键 | 功能 |
| --- | --- |
| 上 / 下 | 左栏导航中切换分类（直播源/画面比例/解码/超时/偏好/更新/调试/关于/退出） |
| 右 | 焦点移到右侧内容区 |
| 左 | 焦点移回左栏导航 |
| OK | 确认选择/切换开关 |

## 支持的源格式

| 类型 | 说明 |
| --- | --- |
| M3U / M3U8 | 标准播放列表（URL 或本地文件） |
| TXT | 每行 `频道名,流地址`（支持中文逗号、纯地址行） |
| 直播链接 | 单个 http/https 流地址直接播放 |
| EPG | XMLTV 格式节目单（可选） |

## 安装

1. 在 **Releases** 页面下载最新 `iptv-player-vN.apk`（或从 **Actions** 页面最新一次构建下载）
2. 将 APK 复制到电视（U 盘 / Send files to TV / adb install 均可）
3. 电视上开启"允许安装未知来源应用"，安装即可
4. 以后新版本**直接覆盖安装**，播放列表、收藏、配置全部保留

> 推送到 `main` 分支会自动触发 GitHub Actions 构建并发布 Release（Release 列表只保留最近 3 个）。
>
> **签名说明**：构建使用固定 debug 签名（keystore 保存在仓库 `.github/debug-keystore.b64` 与 Actions 缓存）。签名固定后，**所有新版本都可以直接覆盖安装，配置保留**。

## 在 GitHub 上构建

推送到 `main` 分支会自动触发 GitHub Actions 构建 APK（也可以在 Actions 页面手动 Run workflow）。

- 构建产物：`assembleRelease`，用 debug 密钥签名，可直接侧载安装（自用足够）
- 构建成功后自动发布 Release 并只保留最近 3 个版本
- 构建失败时自动将错误日志提交为 `build-errors.txt` 到仓库根目录
- 如需正式发布签名：在 `app/build.gradle.kts` 的 `release` 配置自己的 keystore

## 本地构建（可选）

需要 JDK 17 + Android SDK（compileSdk 36）+ Gradle 8.9：

```bash
gradle assembleRelease
# APK 输出: app/build/outputs/apk/release/app-release.apk
```

## 使用

1. 打开应用 → 按遥控器**菜单键**唤出右侧设置面板
2. 在"直播源"页填入播放列表地址，或点**导入本地文件**从本地导入
   - 支持配置多个直播源，在源列表中点击切换
   - 点**一键测速**检测各源延迟，开启"自动优选最快源"后测速完成自动切换到最快源
3. 可选：填入 EPG 节目单地址（支持多个，每行一个）
4. 返回播放界面，按 **OK 键**唤出频道列表：
   - 左栏是分组（央视/卫视/...），上下键切换分组，右栏自动显示对应分组的频道
   - 按右键进入右栏频道列表，OK 键播放选中频道
   - 按左键回到左栏分组
5. 也可在设置页点**扫码管理**，用手机扫码在浏览器里管理直播源与节目指南
6. 全屏播放时：上下键换台，左右键换线路，菜单键唤设置，双击返回退出

## 技术栈

- Kotlin 2.2.10 + AndroidX (AppCompat / RecyclerView / ViewBinding)
- **Media3 ExoPlayer 1.11.0**（HLS 专业支持，H.265/HEVC 硬解）
- **NanoHTTPD**（局域网管理服务）+ **ZXing**（二维码）
- minSdk 23（Android 6.0）/ targetSdk 34 / compileSdk 36

## 致谢与许可

本项目参考了多个开源项目，部分代码思路来源于以下项目（均已在代码注释中标明出处），特此致谢：

- 播放内核：**Media3 ExoPlayer**（Google 开源，Apache License 2.0）
  仓库：https://github.com/androidx/media
- 界面图标：**Material Design Icons**（Apache License 2.0）
- 解码兼容性方案参考：**lemonTV**（MIT License）
  仓库：https://github.com/jia070310/lemonTV
  参考内容：其播放器开启解码器回退（Decoder Fallback）以兼容 H.265 高清源的配置思路；本项目代码为独立编写，未复制其源码。
- 局域网管理服务：**NanoHTTPD**（BSD 3-Clause）
  仓库：https://github.com/NanoHttpd/nanohttpd
- 二维码生成：**ZXing**（Apache License 2.0）
  仓库：https://github.com/zxing/zxing

本播放器不内置任何直播源，仅播放用户提供的播放列表。

## 说明

- 本应用只播放你提供的播放列表，不内置任何直播源
- 部分运营商标记的源通常**只能在对应运营商网络下播放**，与播放器无关
- H.265/HEVC 频道依赖电视硬件解码能力；部分老盒子偶发硬解异常，重启盒子可恢复
- 自动更新下载走双线：先直连 GitHub，失败自动切换 gh-proxy 加速中转
